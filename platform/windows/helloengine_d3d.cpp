...
void InitPipeline() {
    ...
    D3DReadFileToBlob(L"comp.vso", &VS);
    D3DReadFileToBlob(L"comp.pso", &PS);

    // encapsulate both shaders into shader objects
    g_pDev->CreateVertexShader(VS->GetBufferPointer(), VS->GetBufferSize(), NULL, &g_pVS);
    g_pDev->CreatePixelShader(VS->GetBufferPointer(), PS->GetBufferSize(), NULL, &g_pPS);
...
int WINAPI WinMain(
    ...
    hWnd = CreateWindowEx(0,
                          ...
                          NULL,                                     // we aren't using munus, NULL
